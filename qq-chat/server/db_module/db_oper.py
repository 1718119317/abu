# db_oper.py
# pymysql数据库访问类
from server.db_module.db_conf import *
import pymysql
...
class DBOper:
    ...
    def open_conn(self): #连接数据库
        try:
            self.db_conn = pymysql.connect(host, user, password, dbname,use_unicode=True,charset='utf8')
...
if __name__ == "__main__":
    dboper = DBOper()
    dboper.open_conn()
    ret = dboper.do_query("select * from users")
    for x in ret:
        print(x)
